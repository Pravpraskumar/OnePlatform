...
export function CreditGuardNewRequestPage() {
  const api = useApi();
  ...
  const [details, setDetails] = useState<RequestDetailsForm>({
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!details.legalLanguageConfirmed) {
    ...
    setErrorMessage('');
    try {
      await creditGuardRequest('/requests', {
        method: 'POST',
        body: JSON.stringify({
          ...
          requestedBy: user?.displayName ?? details.requesterName,
      ...
      navigate(requestsUrl, { replace: true });
    } catch (error) {
  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-6xl space-y-5">
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-slate-500">
        <Link to={requestsUrl} className="hover:text-brand">Requests</Link><ChevronRight size={14} /><span className="font-medium text-slate-900">New Request</span>
      </nav>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="flex items-center gap-2"><FilePlus2 size={22} className="text-brand" /><h1 className="text-2xl font-semibold text-slate-900">New Company Guarantee Request</h1></div><p className="mt-1 text-sm text-slate-500">Complete the request and approval information for Corporate Treasury.</p></div>
        <div className="flex gap-2"><Button type="button" variant="secondary" onClick={() => navigate(requestsUrl)}>Cancel</Button><Button type="submit" disabled={saving}><Save size={16} />{saving ? 'Saving...' : 'Save request'}</Button></div>
      </div>
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <label className={labelClass}>Workflow status<select className={inputClass} value={summary.status} onChange={(event) => setSummary({ ...summary, status: event.target.value })}><option>Draft</option><option>Under Review</option></select></label>
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <TextField label="Beneficiary of guarantee" value={summary.beneficiary} onChange={(value) => setSummary({ ...summary, beneficiary: value })} required />
      ...
      <Card>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2"><TextAreaField label="Attachments" value={details.attachments} onChange={(value) => updateDetail('attachments', value)} rows={2} readOnly /></div>
        </div>
      </Card>
      ...
      <div className="flex justify-end gap-2 pb-6"><Button type="button" variant="secondary" onClick={() => navigate(requestsUrl)}>Cancel</Button><Button type="submit" disabled={saving}><Save size={16} />{saving ? 'Saving...' : 'Save request'}</Button></div>
    </form>
  );
}